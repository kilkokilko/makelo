...
export default function KineticGrid({
  ...
  // The background dot texture never changes between resizes, so it is baked
  // once into an offscreen canvas instead of re-stroking thousands of arcs
  // every single frame.
  const dotsRef = useRef<HTMLCanvasElement | null>(null);
  // When the cursor has settled and no ripples are alive the grid is static —
  // there is nothing to repaint, so we skip the frame entirely.
  ...
  const draw = useCallback(
    (now: number) => {
      ...
      // Static background dot texture (pre-baked, see dotsRef)
      if (dotsRef.current) ctx.drawImage(dotsRef.current, 0, 0);
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      const moving = Math.abs(m.x - t.x) > 0.4 || Math.abs(m.y - t.y) > 0.4;
      const busy = moving || ripplesRef.current.length > 0;

      // Repaint only while something is actually in motion; once the grid has
      // settled it simply holds the last painted frame for free.
      if (busy || !settledRef.current) {
        draw(now);
        settledRef.current = !busy;
      }

      rafRef.current = requestAnimationFrame(animate);
    };

    // Scoped to the host element (not the viewport) so the grid can be used as
    // one band inside a longer page instead of a fixed full-screen backdrop.
    const bakeDots = (w: number, h: number) => {
    ...
    // The grid only animates while the hero is on screen and the tab is
    // visible — scrolled past, it costs nothing.
    const start = () => {
