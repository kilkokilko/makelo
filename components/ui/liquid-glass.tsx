...
interface DockIcon {
  /** Bitmap icon. Omit when supplying `node` instead. */
  src?: string;
  alt: string;
  label?: string;
  /** Vector icon (e.g. a lucide-react icon) rendered in place of `src`. */
      ...
      {icons.map((icon, index) => {
        ...
        return icon.node ? (
          <div
            ...
            aria-label={icon.alt}
            className="grid size-14 sm:size-16 place-items-center rounded-2xl bg-white/25 text-white shadow-[inset_0_1px_0_rgba(255,255,255,.45)] transition-all duration-700 hover:scale-110 cursor-pointer"
            style={motionStyle}
